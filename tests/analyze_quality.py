...
def main():
    ...
    if manifest_path.exists():
        for line in open(manifest_path).readlines()[1:]:
            parts = line.strip().split(",")
            if len(parts) >= 9:
                mname = parts[8].split("/")[0] if "/" in parts[8] else parts[8]
                manifest_map[mname] = {
                    "motion_type": parts[1],
                    "vx": float(parts[4]) if len(parts) > 4 else 0,
                    "vy": float(parts[5]) if len(parts) > 5 else 0,
                    "wz": float(parts[6]) if len(parts) > 6 else 0,
                }
